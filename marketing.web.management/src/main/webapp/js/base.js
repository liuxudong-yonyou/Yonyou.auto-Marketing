...
var BenFrame={
	...
	checkLogin:function(data){//检查是否登录超时
		if(data.logoutFlag){
			...
			BenFrame.alert('提示',"登录超时,点击确定重新登录.",'error',BenFrame.toLogin);
			return false;
		}
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!BenFrame.checkLogin(data)){
					 		...
					 		BenFrame.alert('提示', data.msg || "请求出现异常,请联系管理员",'error');
					 	}
			 		}catch(e){
			 			BenFrame.alert('提示',"请求出现异常,请联系管理员.",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
			var option =
			{
			 ...
			 BenFrame.ajaxSubmit(form,option);
	},
	saveForm:function(form,callback){
		if(form.form('validate')){
			BenFrame.progress('Please waiting','Save ing...');
			//ajax提交form
	...
	getById:function(url,option,callback){
		BenFrame.progress();
		BenFrame.ajaxJson(url,option,function(data){
			BenFrame.closeProgress();
			if(data.success){
				if(callback){
			       	callback(data);
